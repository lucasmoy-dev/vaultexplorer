 ...
 * that release as an asset. This app looks for `ytpocket-vX.Y.Z`, takes the
...
object Updater {
    ...
    fun download(context: Context, url: String, onProgress: (Float) -> Unit): File {
        ...
        val target = File(dir, "ytpocket-update.apk")
        val part = File(dir, "ytpocket-update.apk.part")
        val connection = (URL(url).openConnection() as HttpURLConnection).apply {
